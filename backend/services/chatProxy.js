const { readErrorMessage, explainProviderError, sanitizeMessages, joinUrl, getCustomProvider } = require('../utils');
...
async function streamChat(req, res, cfg) {
  ...
  try {
    if (provider === 'anthropic' || (addedEndpoint && addedEndpoint.id === 'anthropic')) {
    ...
    } else if (provider === 'openai' || provider === 'groq' || provider === 'openrouter' || provider === 'deepseek' || customProvider || (addedEndpoint && addedEndpoint.id !== 'gemini' && addedEndpoint.id !== 'ollama')) {
      const endpoints = {
    ...
    } else if (provider === 'gemini' || (addedEndpoint && addedEndpoint.id === 'gemini')) {
