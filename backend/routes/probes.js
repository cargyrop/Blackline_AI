...
async function postProbe(req, res) {
  ...
  await run('evolvePlan', async () => {
    const out = await runSelfChatProbe(port, provider, model, 'Return exactly one fenced code block tagged plan containing a JSON array with one object: {"path":"public/app.js","action":"edit","description":"probe only"}. No other text.');
  ...
  await run('evolvePatch', async () => {
    const out = await runSelfChatProbe(port, provider, model, 'Return only valid JSON for a search/replace patch: {"path":"public/app.js","action":"edit","changes":[{"search":"const probe = false;","replace":"const probe = true;"}]}. No markdown.');
    const parsed = extractFirstJson(out);
