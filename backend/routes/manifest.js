...
router.get('/', (req, res) => {
  try {
    ...
    res.json({
      ...
      version: pkg.version,
      techStack: ['Node.js + Express backend (modular: backend/routes/ + backend/services/ + backend/middleware/ + backend/config/ + backend/utils/)', 'vanilla HTML/CSS/JS frontend (public/index.html + public/styles.css + public/app.js)', 'no build step', 'Server-Sent Events for streaming chat and updates'],
      port: 3737,
      storage: {
