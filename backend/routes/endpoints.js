...
const { readErrorMessage, joinUrl, enrichModel, prettyModelName } = require('../utils');
...
async function fetchCatalogForEndpoint({ provider, baseUrl, apiKey, category }) {
  ...
  if (provider === 'anthropic') {
    ...
    const targetUrl = cleanBase ? joinUrl(cleanBase, '/api/tags') : 'http://localhost:11434/api/tags';
...
// POST /api/endpoints
router.post('/', (req, res) => {
  const { id, label, icon, baseUrl, apiKey, category, models } = req.body || {};
  ...
  const endpointData = {
    ...
    enabled: true,
    disabledModels: existingIndex !== -1 ? (cfg.endpoints[existingIndex].disabledModels || []) : [],
    updatedAt: Date.now()
  };
