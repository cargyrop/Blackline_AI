...
describe('backend/services/evolveEngine', () => {
  ...
  describe('streamModelText', () => {
    it('streams through added OpenAI-compatible endpoints', async (t) => {
      ...
      global.fetch = async (url) => {
        ...
        return {
          ok: true,
          body: [Buffer.from('data: {"choices":[{"delta":{"content":"OK"}}]}\n\n')]
        };
      };
      ...
      const result = await streamModelText(
        ...
        'Say OK',
        txt => { streamed += txt; },
        { endpoints: [{ id: 'mistral', baseUrl: 'https://api.mistral.ai/v1', apiKey: 'mk-test', enabled: true }] },
        () => {}
      );

      assert.strictEqual(calledUrl, 'https://api.mistral.ai/v1/chat/completions');
      assert.strictEqual(streamed, 'OK');
