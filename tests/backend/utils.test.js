...
describe('backend/utils', () => {
  ...
  describe('safeResolve', () => {
    ...
    it('allows valid relative paths', () => {
      assert.strictEqual(safeResolve(base, 'frontend/app.js'), '/home/user/app/frontend/app.js');
    ...
    it('blocks .. traversal', () => {
      ...
      assert.strictEqual(safeResolve(base, 'frontend/../../etc/passwd'), null);
    });
    ...
    it('blocks null bytes', () => {
      assert.strictEqual(safeResolve(base, 'frontend/app.js\0'), null);
    });
  ...
  describe('toPosixPath', () => {
    it('converts backslashes', () => {
      assert.strictEqual(toPosixPath('frontend\\app.js'), 'frontend/app.js');
    });
  ...
  describe('ollamaBaseUrl', () => {
    it('defaults to IPv4 loopback to avoid localhost resolution issues', () => {
      assert.strictEqual(defaultOllamaBaseUrl(), 'http://127.0.0.1:11434');
    });

    it('uses the configured Ollama endpoint base URL when present', () => {
      const cfg = { endpoints: [{ id: 'ollama', providerType: 'ollama', baseUrl: 'http://192.168.1.20:11434/', enabled: true }] };
      assert.strictEqual(ollamaBaseUrl(cfg), 'http://192.168.1.20:11434');
    });

    it('returns fallback candidates including localhost for compatibility', () => {
      assert.deepStrictEqual(ollamaBaseUrlCandidates({}), ['http://127.0.0.1:11434', 'http://localhost:11434']);
    });
  });
