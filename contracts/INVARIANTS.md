...
## 10. No Shell Execution

- The backend must never use `exec()`, `spawn()`, `child_process` for anything other than opening the browser on startup (`start.sh` / `START.bat` logic).
- No file execution, no `eval()`, no dynamic code loading.
- The only file operations are: read, write, delete, copy (for backups).
