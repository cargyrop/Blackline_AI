/* BLACKLINE AI — settings module (Phase 2 + Odysseus Unified Endpoints) */

let lastTestedEndpoint = null;
let addedEndpointsList = [];

function onOdysseusProviderChange() {
  const sel = document.getElementById('odysseus-provider');
  const urlIn = document.getElementById('odysseus-url');
  if (!sel || !urlIn) return;
  const opt = sel.options[sel.selectedIndex];
  const defaultUrl = opt?.getAttribute('data-url') || '';
  if (defaultUrl) urlIn.value = defaultUrl;
  const addBtn = document.getElementById('odysseus-add-btn');
  if (addBtn) addBtn.disabled = true;
  const status = document.getElementById('odysseus-status-line');
  if (status) status.innerHTML = '<span class="status-offline">Offline</span> — enter API key and click Test to discover models';
}

async function testOdysseusEndpoint() {
  const providerSel = document.getElementById('odysseus-provider');
  const urlIn = document.getElementById('odysseus-url');
  const keyIn = document.getElementById('odysseus-key');
  const catSel = document.getElementById('odysseus-category');
  const testBtn = document.getElementById('odysseus-test-btn');
  const addBtn = document.getElementById('odysseus-add-btn');
  const status = document.getElementById('odysseus-status-line');
  ...
  const category = catSel?.value || 'LLM';
...
async function testLocalEndpoint() {
  ...
  try {
    const r = await fetch('/api/endpoints/test', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ provider: 'ollama', baseUrl: 'http://localhost:11434', apiKey: 'none' })
    });
    const data = await r.json();
    ...
    await fetch('/api/endpoints', {
      ...
      body: JSON.stringify({
        ...
        baseUrl: 'http://localhost:11434', apiKey: 'local',
        category: 'LLM', models: data.models || []
      })
    });
...
async function addOdysseusEndpoint() {
...
async function renderAddedEndpoints() {
  const container = document.getElementById('odysseus-added-list');
  if (!container) return;
  ...
  if (!addedEndpointsList || !addedEndpointsList.length) {
    container.innerHTML = '<div class="odysseus-empty-hint">No endpoints added yet. Select a provider above, test, and click Add.</div>';
    return;
  }
    ...
    card.innerHTML = `
      <div class="endpoint-card-main">
        ...
        <div class="endpoint-card-actions">
          <button class="btn-ody-disable" onclick="toggleEndpointStatus('${escHtml(ep.id)}')">${isEnabled ? 'Disable' : 'Enable'}</button>
          <button class="btn-odysseus-delete" onclick="deleteOdysseusEndpoint('${escHtml(ep.id)}')">Delete</button>
          <button class="btn-odysseus-expand" onclick="toggleEndpointModels('${escHtml(ep.id)}')">˅</button>
        </div>
      </div>
      <div class="endpoint-models-drawer" id="drawer-${escHtml(ep.id)}" style="display:none;">
        <div class="drawer-toolbar">
          <input type="text" class="drawer-filter" placeholder="Filter models..." oninput="filterDrawerModels('${escHtml(ep.id)}', this.value)" />
...
async function deleteOdysseusEndpoint(id) {
...
/* Backwards compatibility stubs for legacy references */
