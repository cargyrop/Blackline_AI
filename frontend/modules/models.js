...
function modelMatchesCenterFilter(m) {
  const searchQ = (document.getElementById('catalog-search-input')?.value || '').toLowerCase();
  if (searchQ && !(m.name || m.id).toLowerCase().includes(searchQ) && !m.provider.toLowerCase().includes(searchQ)) return false;
  ...
  if (modelCenterFilter === 'recommended') return (m.arena?.textElo || 0) >= 1230;
  if (modelCenterFilter === 'free') return ['local', 'free'].includes(pricing);
  if (modelCenterFilter === 'evolve') return m.updateCapable;
  if (modelCenterFilter === 'vision') return !!caps.imageInput;
  if (modelCenterFilter === 'pinged') return probe?.type === 'ping' || probe?.status === 'pass';
  return true;
}
...
function renderModelCenter() {
  ...
  if (!filtered.length) {
    container.innerHTML = '<div class="model-center-empty">No models match your search or filter. Add remote endpoints above.</div>';
    return;
  }
  ...
  for (const [prov, ms] of Object.entries(provGroups)) {
    ...
    ms.sort((a, b) => {
      const aElo = a.arena?.textElo || 1000;
      const bElo = b.arena?.textElo || 1000;
    ...
    const allProvEnabled = enabledInProv === totalInProv;
    ...
    block.innerHTML = `
      <div class="tp-provider-header" onclick="toggleProviderTable('${escHtml(prov)}')">
        <div class="tp-provider-title-wrap" onclick="event.stopPropagation()">
          <input type="checkbox" class="tp-checkbox" ${allProvEnabled ? 'checked' : ''} onchange="toggleEntireProvider('${escHtml(prov)}', this.checked)" title="Toggle entire endpoint" />
    ...
    for (const m of ms) {
      ...
      const textEloTxt = m.arena?.matched ? `ELO ${m.arena.textElo}` : `EST ${m.arena?.textElo || 1150}`;
      const codeEloTxt = m.arena?.matched ? `ELO ${m.arena.codingElo}` : `EST ${m.arena?.codingElo || 1130}`;
      ...
      tr.innerHTML = `
        ...
        <td><strong>${escHtml(m.name || m.id)}</strong><span style="font-family:var(--font-mono); color:var(--text-dim); font-size:10px;">${escHtml(m.id)}</span></td>
